...
namespace ml::midi {
    ...
    public:
        ...
        using RawMidiMessage = std::uint32_t;

        MidiMessage() = default;

        explicit MidiMessage(const RawMidiMessage msg);

        [[nodiscard]] RawMidiMessage raw() const;
        ...
        [[nodiscard]] std::uint8_t note() const;
        [[nodiscard]] std::uint8_t velocity() const;
        [[nodiscard]] std::uint8_t control() const;
        [[nodiscard]] std::uint8_t value() const;
        [[nodiscard]] std::uint8_t program() const;

        // 14-bit pitch bend centered at 0
        [[nodiscard]] std::int16_t pitchBend() const;

    private:
        void parse();

        RawMidiMessage raw_{0};
